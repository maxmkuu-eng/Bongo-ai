...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && pathname === '/api/health') {
    return send(res, 200, { ok: true, service: 'bongo-ai', aiConfigured: Boolean(process.env.GEMINI_API_KEY) });
  }
  ...
  if (req.method === 'POST' && pathname === '/api/chat') {
    ...
    req.on('end', async () => {
      try {
        ...
        return send(res, 502, { error: `Gemini request failed: ${error?.message || 'Unknown provider error'}` });
      }
    });
    return;
  }
